...
!--------------------------------------------------------------
!
! generic body of the subroutines convert_ring2nest_*_nd 
...
!--------------------------------------------------------------
    ...
    if (nd == 1) then
       map1 => map(0:npix-1,1)
       call convert_ring2nest(nside, map1)
       return
    endif
...
!$OMP end parallel
       ...
       do j = 1, nd
          do ipr4 = 0_i4b, npix-1
             map_tmp(mapping4(ipr4)) = map(ipr4,j)
          enddo
...
!$OMP end parallel
       ...
       do j = 1, nd
          do ipr8 = 0_i8b, npix-1
             map_tmp(mapping8(ipr8)) = map(ipr8,j)
          enddo
          map(0:npix-1, j) = map_tmp(0:npix-1)
       enddo
