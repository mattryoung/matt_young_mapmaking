...
contains
  ...
  subroutine makemaptable(fileout,model,nu,alphaCMB)
    ...
    real nu, alphaCMB
    real x0const  ! =0.01762 h/k_b/T_CMB in units of 1/GHz
    real x0new
    real Gx       !G(x)
    ...
    x0const = 0.017612 * nu ! so when multiplied by nu*(1+z)^alpha in GHz this will give the usual dimensionless frequency
    ...
    write(*,*) ""
    write(*,*) "Making Maptable. Will take a minute"
    write(*,*) ""
    do i=1,nchit
       ...
       if(nu>0.0) then
          x0new = x0const*(1+z)**alphaCMB
          Gx    = x0new * (exp(x0new)+1)/(exp(x0new)-1) - 4
       endif
       ...
       do j=1,nmht
          ...
          do k=1,nrt
             ...
             if(nu>0.0) y = y*Gx !converting y to Delta T/T_CMB
